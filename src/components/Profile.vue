    ...
    <div class="bg-gray-300 rounded-lg shadow-2x1 w-full -mt-40">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-gray-100 divide-y divide-gray-200">
          <tr v-for="orders in page.content" :key="orders">
            ...
            <td class="px-16 py-2 whitespace-nowrap">
              <div class="text-sm text-gray-900">{{ orders.stockSymbol }}</div>
            </td>
            <td class="px-16 py-2 whitespace-nowrap" v-if="orders.type == 1">
              Venda
            </td>
            <td class="px-16 py-2 whitespace-nowrap" v-else>Compra</td>
            <td class="px-16 py-2 whitespace-nowrap">
              {{
    ...
    async getPage() {
      ...
      try {
        let response = await axios.get(
          `http://localhost:8081/page?pageSize=10&pageNumber=${this.pg}&sort=id_order,asc`,
          {
